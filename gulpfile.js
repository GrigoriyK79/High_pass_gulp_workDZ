...
const scripts = () => {
    ...
    .on('error', function(err) {
        ...
        this.emit('end'); // Don't stop the rest of the task, Не останавливайте оставшуюся часть задачи
    })
...
const tinypng = () => {
    ...
    .pipe(tiny({
        ...
        parallel: true,
        parallelMax: 50, // Влияет на количество обрабатываемых фотографий
        log: true
    }))
    .pipe(dest('./app/img'))
}
...
const deploy = () => {
	...
	return src(globs, {
		...
		.pipe(conn.newer('')) // only upload newer files, загружать только новые файлы
		.pipe(conn.dest(''));
}
